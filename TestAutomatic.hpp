   ...
   public:
    ...
    void insertFakeUsers(std::ostream &anOut,
        ...
        for (size_t theCount = 0; theCount < aGroupCount; theCount++) {
            ...
            for (size_t theSize = 0; theSize < aGroupSize; theSize++) {
                anOut << thePrefix << '(' << '"' << Fake::People::first_name()
    ...
    bool doUpdateTest() {
        ...
        if (theResult) {
            ...
            Expected  theExpected({{Commands::createDB, 1}, {Commands::createDB, 1}, {Commands::useDB, 0}, {Commands::createTable, 1}, {Commands::insert, 5}, {Commands::useDB, 0}, {Commands::dropDB, 0}, {Commands::useDB, 0}, {Commands::select, 5}, {Commands::update, 1}, {Commands::select, 1}, {Commands::dropDB, 0}});
    ...
    bool doIndexTest() {
        ...
        if (theResult) {
            ...
            Expected  theExpected({
                ...
                {Commands::dropTable, 15},
                {Commands::showIndexes, 1},
                {Commands::useDB, 0},
                {Commands::dropDB, 0},
                {Commands::useDB, 0},
                {Commands::select, 5},
    ...
    bool doIOTest(double &anElapsed, char aPrefix) {
        ...
        myStream.open("cmd.txt", std::fstream::binary | std::fstream::in | std::fstream::out | std::fstream::trunc);
        myStream.close();
        myStream.open("cmd.txt", std::fstream::binary | std::fstream::binary | std::fstream::in | std::fstream::out);
        myStream.write(temp.c_str(), temp.size());
        myStream.close();
        std::stringstream theInput(theStream1.str());
        std::stringstream theOutput;
    ...
    bool doCacheTest(CacheType aType, size_t aCapacity) {
        ...
        if (theResult) {
            double theAvgTime = std::accumulate(
                                    theTimes.begin(), theTimes.end(), 0.0) /
                                theTimes.size();
